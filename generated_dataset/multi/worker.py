...
parser.add_argument("--max_num_dynamic_objects", type=int, default=1,
...
parser.set_defaults(save_state=False, frame_end=32, frame_rate=12,
                    resolution=256)
...
# randomly select direction form 'right' or 'left'
if rng.rand() < 0.5:
    direction = "right"
else:
    direction = "left"
...
if direction == "left":
  DYNAMIC_SPAWN_REGION = [(1, -5, 2), (5, 5, 5)]
  VELOCITY_RANGE = [(-1.5, -2., 0.), (-0.5, 2., 0.)]
else:
  DYNAMIC_SPAWN_REGION = [(-5, -5, 2), (-1, 5, 5)]
  VELOCITY_RANGE = [(0.5, -2., 0.), (1.5, 2., 0.)]

ii = str(FLAGS.ii).zfill(5)

output_dir = Path(FLAGS.output_dir)
saving = output_dir/"rawdata"/str(ii)
...
# --- Populate the scene
# background HDRI
with open('generated_dataset/multi/backgrounds.json') as f:
    backgrounds_split = json.load(f)
logging.info("Choosing one of the %d held-out backgrounds...", len(backgrounds_split))
hdri_id = rng.choice(backgrounds_split)
...
if FLAGS.camera == "fixed_random":
  ...
  # scene.camera.position = [0, -5, 5.34367]
  # while scene.camera.position[1] > -2:
  #   scene.camera.position = kb.sample_point_in_half_sphere_shell(
  #     inner_radius=7., outer_radius=9., offset=0.1)
  scene.camera.position = rng.uniform((-2, -7, 5), (2,-5,7))
...
active_split = json.load(open('generated_dataset/multi/objects.json'))
...
for i in range(num_dynamic_objects):
  ...
  R = rotation_matrix_2d([0, -1], [scene.camera.position[0],scene.camera.position[1]])
  new_vel = np.dot(R, [obj.velocity[0],obj.velocity[1]])
  obj.velocity = [new_vel[0], new_vel[1], obj.velocity[2]]
...
# kb.write_image_dict({"rgba":data_stack["rgba"]}, output_dir/"rawdata"/str(ii))
kb.write_image_dict(data_stack, output_dir/"rawdata"/str(ii))
write_video(data_stack["rgba"], output_dir/"videos"/f"video_{ii}.mp4", fps = 12)
kb.post_processing.compute_bboxes(data_stack["segmentation"],
                                  visible_foreground_assets)

# --- Metadata
logging.info("Collecting and storing metadata for each object.")
kb.write_json(filename=output_dir /"rawdata"/ f"metadata_{ii}.json", data={
    "flags": vars(FLAGS),
...
kb.write_json(filename=output_dir / "rawdata"/ f"events_{ii}.json", data={
