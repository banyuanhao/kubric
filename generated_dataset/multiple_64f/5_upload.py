...
def create_webdataset_image(input_args):
    ...
    for video_filename in tqdm(video_filenames):
        ...
        if shard_size > shard_size_mb:
            ...
            s3.upload_file(os.path.join(SHARD_DIR, shard_filename), "snap-webdataset-videos", "multiple_64f/" + shard_filename)
...
if __name__ == "__main__":
    ...
    for split in lst:
        ...
        if split == "train_video_aug":
            ...
            image_selecting = video_list.image_selecting.tolist()
            video_list = video_list.id.tolist()
            video_list = [id for id in video_list for _ in range(8)]
            image_selecting = [eval(i) for i in image_selecting]
            image_selecting = [item for sublist in image_selecting for item in sublist]
            video_list = ["multi_image_%s_%s.png" % (str(vn).zfill(5), str(ims).zfill(5)) for vn, ims in zip(video_list, image_selecting)]
