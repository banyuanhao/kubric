...
PROC_VIDEO_DIR = os.path.join(DATA_DIR, 'videos_our_format_two_direction')
os.makedirs(PROC_VIDEO_DIR, exist_ok=True)

# Load the ActivityNet captions
TRAIN_CAPS = load_json(os.path.join(DATA_DIR, 'videos_train_two_direction.json'))
VAL_CAPS = load_json(os.path.join(DATA_DIR, 'videos_val_two_direction.json'))
...
def process_video(video_path):
    ...
    vidinfo["dataset"] = "synthetic_one_direction_video_and_image_two_direction"
    vidinfo["pre_encode_video_filename"] = video_name + ".mp4"
    vidinfo["filename"] = "synthetic_one_direction_video_and_image_two_direction_%s.mp4" % (video_name)
    ...
    if not os.path.exists((video_path.replace(".mp4", "_embeddings.pkl"))):
        path = video_path.replace(".mp4", "_embeddings.pkl")
...
if __name__ == "__main__":
    ...
    video_paths = [name for name in video_paths if (int(os.path.basename(name).split('_')[1].split('.')[0]) < 24000) or (int(os.path.basename(name).split('_')[1].split('.')[0]) > 32000 and int(os.path.basename(name).split('_')[1].split('.')[0]) < 56000)]
