...
VIDEO_DIR = os.path.join(DATA_DIR, 'videos_our_format_two_direction')
SHARD_DIR = os.path.join(DATA_DIR, 'video_shards_our_format_two_direction')
...
def create_webdataset(input_args):
    ...
    for video_filename in tqdm(video_filenames):
        ...
        if shard_size > shard_size_mb:
            ...
            s3.upload_file(os.path.join(SHARD_DIR, shard_filename), "snap-webdataset-videos", "synthetic_two_direction_video/" + shard_filename)
...
if __name__ == "__main__":
    ...
    if args.split == "train":
        ...
        video_list = ["synthetic_one_direction_video_and_image_two_direction_%s.mp4" % (vn[2:]) for vn in video_list]
