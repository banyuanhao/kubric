...
def reencode_image(image_path, output_name):
    os.system("""/usr/bin/ffmpeg -hide_banner -loglevel quiet -i %s -vf "scale='min(1024,iw)':-2" -q:v 2 %s""" % (image_path, output_name))
    width, height = get_resolution_image(output_name)
    return int(width), int(height)

def get_resolution_image(image_path):
    import subprocess
    result = subprocess.run(["/usr/bin/ffprobe", "-v", "error", "-select_streams", "v:0", "-show_entries", "stream=width,height", "-of", "csv=s=x:p=0", image_path], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    width, height = result.stdout.decode('utf-8').strip().split('x')
    return width, height
...
def get_image_information(video_path):
    result = {}

    # get dimention
    result["width"], result["height"] = get_resolution_image(video_path)
